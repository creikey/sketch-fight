...
func _ready():
	game_state.connect("farmer_destroyed", self, "on_farmer_destroyed")

func on_farmer_destroyed():
	...
	# assign a winner position to the one with farmers while everybody else doesn't have any
	var won_id = -1
	for p in game_state.get_player_ids():
		if game_state.get_player_gamestate(p)[0] > 0: # this player has active farmers
			if won_id > 0: # another player has farmers too
				return
			won_id = p
